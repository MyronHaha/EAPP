...
import com.hyhscm.myron.eapp.activity.Im.db.DemoDBManager;
import com.hyhscm.myron.eapp.activity.Im.db.InviteMessgeDao;
import com.hyhscm.myron.eapp.activity.Im.db.UserDao;
import com.hyhscm.myron.eapp.activity.Im.domain.InviteMessage;
import com.hyhscm.myron.eapp.activity.Im.domain.RobotUser;

import com.hyhscm.myron.eapp.activity.Im.utils.PreferenceManager;
...
//			getUserProfileManager().init(context);
    ...
    private EMOptions initChatOptions(){
        ...
        // set if accept the invitation automatically   好友请求
...
// 通话
    ...
    protected void setEaseUIProviders() {
        //set user avatar to circle shape   设置头像形状
...
//        });
        ...
        easeUI.getNotifier().setNotificationInfoProvider(new EaseNotificationInfoProvider() {
            ...
            public String getDisplayedText(EMMessage message) {
                ...
                String ticker = EaseCommonUtils.getMessageDigest(message, appContext);//txt内容
                if(message.getType() == Type.TXT){
                    ticker = ticker.replaceAll("\\[.{2,3}\\]", "[表情]");
                }
                L.e("jjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjj");
                EaseUser user = getUserInfo(message.getFrom());
                if(user != null){
                    if(EaseAtMessageHelper.get().isAtMeMsg(message)){
                        //user.getnick(); origin
                        return String.format("%s在群聊中@了你", Global.getNickByAll(message.getFrom()).getNname());
                    }
                    return Global.getNickByAll(message.getFrom()).getNname()+ ": " + ticker;//user 不为空返回
                } else{
                    if(EaseAtMessageHelper.get().isAtMeMsg(message)){
                        return String.format("%s在群聊中@了你", Global.getNickByAll(message.getFrom()).getNname());
                    }
                    return Global.getNickByAll(message.getFrom()).getNname() + ": " + ticker;
                }
            }
            ...
            public String getLatestText(EMMessage message, int fromUsersNum, int messageNum) {
                ...
                if(message.ext().containsValue("add")){
                    return Global.getNickByAll(message.getFrom()).getNname()+"请求加你为好友";
                }else if(message.ext().containsValue("accepted")){
                    return Global.getNickByAll(message.getFrom()).getNname()+" 已接受你的好友请求，来发起聊天吧！";
                }else if(message.ext().containsValue("notadd")){
                    try {
                        return "你有"+message.getStringAttribute("num")+"条新的好友请求待处理";
                    } catch (HyphenateException e) {
                        e.printStackTrace();
                    }
                }
            	return Global.getNickByAll(message.getFrom()).getNname()+"发来"+messageNum+"条消息";
            }
...
//                    intent = new Intent(appContext, VoiceCallActivity.class);
                    ...
                    if(message.ext().containsKey("cid")){
                        intent = new Intent(appContext,SysMsg.class);
                    }
                    else if(message.ext().containsValue("add")){
//                        MyToast.makeText(appContext,"新的朋友",1000).show();
                        intent = new Intent(appContext, ContractManager.class);
                    }else if(message.ext().containsValue("accepted")){
                        intent = new Intent(appContext, ChatActivity.class);
                        intent.putExtra(Constant.EXTRA_USER_ID, message.getFrom());
                    }else if(message.ext().containsValue("notadd")){
                        intent = new Intent(appContext,ContractList.class);
                    }
...
//        user.setNickname(getNickByUUid(username));
        return user;
	}

    public static String getNickByUUid(String UUID) {
...
//        List<EMConversation> list = mapTransitionList(map);
        ...
        if (HttpCore.contractList.size() > 0) {
            String nick = UUID;
            for (Contract c : HttpCore.contractList) {

                if (c.getUiid().toLowerCase().equals(UUID)) {
                    nick = c.getNname();
                }
            }
            return nick;
