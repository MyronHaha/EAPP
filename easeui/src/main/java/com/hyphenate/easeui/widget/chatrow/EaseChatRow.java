...
public abstract class EaseChatRow extends LinearLayout {
    ...
    private void setUpBaseView() {
    	// set nickname, avatar and background of bubble   设置 昵称，头像和气泡背景；
        ...
        if(userAvatarView != null) {
            //set nickname and avatar  0129改
            if (message.direct() == Direct.SEND) {
//                EaseUserUtils.setUserAvatar(context, EMClient.getInstance().getCurrentUser(), userAvatarView);
                Glide.with(getContext())
                        .load(Global.IMAGEPATH+"/"+Global.userImg)
                        .error(R.drawable.ease_default_avatar)
                        .into(userAvatarView);;

//                 EaseUserUtils.setUserNick(Global.getNickByAll(message.getTo()).getNname(), usernickView);
                Log.e("send",Global.IMAGEPATH+"/"+Global.userImg);
            } else {
                Glide.with(getContext()).load(Global.IMAGEPATH+"/"+Global.getNickByAll(message.getFrom()).getPic()).error(R.drawable.ease_default_avatar).into(userAvatarView);
                Log.e("receive",Global.IMAGEPATH+"/"+Global.getNickByAll(message.getFrom()).getPic());

//                EaseUserUtils.setUserAvatar(context, Global.IMAGEPATH+Global.getNickByAll(message.getFrom()).getPic(), userAvatarView);
                EaseUserUtils.setUserNick(Global.getNickByAll(message.getFrom()).getNname(), usernickView);
            }
        }
