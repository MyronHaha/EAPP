...
public class EaseChatFragment extends EaseBaseFragment implements EMMessageListener {
    ...
    protected int[] itemStrings = { R.string.attach_take_pic, R.string.attach_picture, R.string.attach_location }; // 拍照，图片，位置
...
//        if(chatType != EaseConstant.CHATTYPE_SINGLE)
            messageList.setShowUserNick(true);   //显示用户昵称
//        messageList.setAvatarShape(1);
    ...
    protected void setUpView() {
        titleBar.setTitle(Global.getNickByAll(toChatUsername).getNname());
        if (chatType == EaseConstant.CHATTYPE_SINGLE) {
            // set title
            if(EaseUserUtils.getUserInfo(toChatUsername) != null){
                EaseUser user = EaseUserUtils.getUserInfo(toChatUsername);
                //nickname
                user.setNickname(Global.getNickByAll(toChatUsername).getNname());
                if (user != null) {
